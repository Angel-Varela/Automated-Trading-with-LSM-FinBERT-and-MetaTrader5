...
class TradingBot():
    ...
    def inner_loop(self):
        ...
        sentiment_1 = estimate_sentiment(news=title_1)
        ...
        now_time = datetime.now().replace(microsecond=0, second=37)
    ...
    def outer_loop(self):
        ...
        while True:
            ...
            if rates.size > 0: #Check if thd DF is empty, if so that means the market is closed right now
                prices = float(rates['close'].iloc[0])
            else:
            ...
            if (nearest_rows["Date"] == now).any():
                print("Running...", now_str, "Predicted Price:", current_price, "Real Price:", prices)                   
                #MARKET IS BULLISH
                ...
                #MARKET IS BEARISH
                        ...
                        if current_price == highest:#If the now's minute is the lowest between the sooner and latest 10
                            ...
                            if current_price == lowest:#If the now's minute is the highest between the sooner and latest 10
                                    if prices == min(rates['close']):
